...
describe("randomCodeGenerator utility functions", () => {
    describe("generateRandomDigitsCode", () => {
        ...
        it("should generate a string of digits of given length", () => {
            const length = 8;
            const code = (0, randomCodeGenerator_1.generateRandomDigitsCode)(length);
            expect(code).toMatch(/^\d{8}$/);
        });
        it("should generate different values on multiple calls", () => {
            const code1 = (0, randomCodeGenerator_1.generateRandomDigitsCode)();
            const code2 = (0, randomCodeGenerator_1.generateRandomDigitsCode)();
    ...
    describe("getHashedDigitCode", () => {
        it("should hash the provided code", async () => {
            const code = "123456";
            const hash = await (0, randomCodeGenerator_1.getHashedDigitCode)(code);
            expect(hash).toMatch(/^\$2[aby]\$.{56}$/); // bcrypt hash format
            const match = await bcryptjs_1.default.compare(code, hash);
            expect(match).toBe(true);
        });
        it("should generate a code if not provided and hash it", async () => {
            const hash = await (0, randomCodeGenerator_1.getHashedDigitCode)(undefined, 6);
    ...
    describe("generateRandomHexCode", () => {
        ...
        it("should generate a hex string of length 2 * byteLength", () => {
            const byteLength = 10;
            const code = (0, randomCodeGenerator_1.generateRandomHexCode)(byteLength);
            expect(code).toMatch(/^[a-f0-9]{20}$/);
        });
        it("should generate different values on multiple calls", () => {
            const code1 = (0, randomCodeGenerator_1.generateRandomHexCode)();
            const code2 = (0, randomCodeGenerator_1.generateRandomHexCode)();
            expect(code1).not.toBe(code2);
        });
    });
    describe("getHashedHexCode", () => {
        it("should return a SHA-256 hash in hex format", () => {
            const code = "strcode";
            const hash = (0, randomCodeGenerator_1.getHashedHexCode)(code);
        ...
        it("same input should return same hash", () => {
            ...
            const hash1 = (0, randomCodeGenerator_1.getHashedHexCode)(code);
            const hash2 = (0, randomCodeGenerator_1.getHashedHexCode)(code);
            expect(hash1).toBe(hash2);
        });
        it("different input should return different hashes", () => {
            const hash1 = (0, randomCodeGenerator_1.getHashedHexCode)("a");
            const hash2 = (0, randomCodeGenerator_1.getHashedHexCode)("b");
