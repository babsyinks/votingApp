...
function Result() {
  ...
  const message =
    "Election Result And Timer Will Be Deleted. You Can Start A New Election After This. Admin Access Is Also Needed. Are You Sure You Want To Proceed?";
  ...
  useEffect(() => {
    const resultStatus = async () => {
      ...
      setResult(res.data.electionData);
    };
    resultStatus();
  }, []);
  ...
  if (result.length) {
    ...
    return (
      <div>
        ...
        <div className="endElection">
          <button onClick={handleElectionEnd}>End This Election</button>
        </div>
        ...
        <div>
          <div className="allResultsWrap">
            {sortedResults.map(({ picture, surname, firstname, votes }, i) => {
              let isTie = false;
              if (i === 0) {
              ...
              return (
                <div className="contestant resultContainer" key={i}>
                  <div className="contestant_picture">
                    <img className="resImg" src={picture} alt="contestant" />
                  </div>
                  <div className="resDetails">
                    ...
                    <div className="aboutVotes">
                      Votes:
                      <span className="votes">{votes.length}</span>
                      <span className="winner">
                        {i === 0 ? (
                          isTie === false ? (
                            ...
                            <span className="resTie">Tie</span>
                          )
                        ) : isTie ? (
                          <span className="resTie">Tie</span>
                        ) : (
