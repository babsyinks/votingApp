...
describe("useTimerSchedule", () => {
  test("should initialize all values as empty strings", () => {
    ...
    const { startDate, startTime, endDate, endTime, mergedTimerState } =
      result.current;
  ...
  test("should update values correctly when onChange is called", () => {
    ...
    act(() => {
      mergedTimerState[0].onChange({
        target: { value: "2025-08-10" },
      } as React.ChangeEvent<HTMLInputElement>);

      mergedTimerState[1].onChange({
        target: { value: "08:00" },
      } as React.ChangeEvent<HTMLInputElement>);

      mergedTimerState[2].onChange({
        target: { value: "2025-08-11" },
      } as React.ChangeEvent<HTMLInputElement>);

      mergedTimerState[3].onChange({
        target: { value: "17:00" },
      } as React.ChangeEvent<HTMLInputElement>);
    });
