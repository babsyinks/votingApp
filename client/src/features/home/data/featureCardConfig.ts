...
const mapContents = (arr: Omit<ContentItem, "id">[]): ContentItem[] =>
  arr.map((item) => ({
    id: uuidv4(),
    ...item,
  }));

const contents1: Omit<ContentItem, "id">[] = [
  {
    icon: Shield,
...
const contents2: Omit<ContentItem, "id">[] = [
  {
    icon: BarChart3,
...
const items: Record<"expect" | "features", FeatureSection> = {
  expect: {
