...
describe("List component", () => {
  ...
  it("renders list items even if the object in the list does not have id", () => {
    const customItems = [{ content: "Item A" }, { content: "Item B" }];
    render(<List items={customItems} />);
    const items = screen.getAllByRole("listitem");
    expect(items).toHaveLength(2);
    expect(items[0]).toHaveTextContent("Item A");
  ...
  it("defaults to empty string if content passed is undefined", () => {
    const customItems = [{ content: undefined }, { content: undefined }];
    render(<List items={customItems} />);
    const items = screen.getAllByRole("listitem");
    expect(items).toHaveLength(2);
