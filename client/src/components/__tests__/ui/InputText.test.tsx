...
describe("<InputText />", () => {
  ...
  const passwordProps: InputTextProps = {
    type: "password",
    name: "password",
    value: "password",
    onChange: jest.fn(),
  ...
  const emailProps: InputTextProps = {
    type: "email",
    name: "email",
  ...
  it("renders the input with correct value and placeholder and defaults to 'text' when no type is provided", () => {
    const basePropsCopy = { ...baseProps };
    delete basePropsCopy.type;
    render(<InputText {...basePropsCopy} />);
  ...
  it("applies only default class names when computed classnames are not passed in", () => {
    mockedGetCompClasses.mockReturnValue("");
  ...
  it("applies default and computed class names", () => {
    mockedGetCompClasses.mockReturnValue("resolved-custom-class");
    render(<InputText {...baseProps} />);
    const input = screen.getByTestId("mock-baseinput");
